...
## Dynamic thinking: Feedback loop
...
## .bg-contrast[Example: An Epidemiological Model]


---
background-image: url("img/batch.jpg")

## .bg-contrast[Example: Resource Limited Growth]
...
## .bg-contrast[Example: Predator-Prey model]


---
background-image: url("img/neunzehnhain.jpg")

## .bg-contrast[Example: A Lake with Inflow and Outflow]

---
background-image: url("img/polluted-river3.jpg")

## .bg-contrast[Example: Nitrification in a Polluted River]
...
* .gray[automata in discrete time: IBMs, ABMs, cellular automata]
* .gray[equations in discrete time: difference equations]
* equations in .blue[continuous time] $\rightarrow$ .blue[differential equations] (ODEs)
* .blue[deterministic] .gray[or stochastic]
...
\frac{dN}{dt} &= \mbox{birth} - \mbox{death}\\
              & = \mbox{birthhrate} \cdot N - \mbox{deathrate}\cdot N\\
...
* The model has derivatives in both dimensions, so we getpartial differential equations (PDE).
...
Fast solvers + efficient estimation the Jacobians + R's vectorization make multidimensional models surprinsingly fast.
...
* longitudilal Diffusion D
* grid cell length dx

---

## Full Source Code (with Animation)

```{r river-1d-full, eval=FALSE}
library("ReacTran")
library("deSolve")
library("rootSolve")
library("gifski")
