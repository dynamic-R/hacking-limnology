...
The source code of these slides can be found [here](https://github.com/dynamic-R/hacking-limnology).
...
par(mar=c(2.5, 5, 0, 1)+.1, las=1, lwd=2)
r    <- 0.5
N0   <- 10
dt   <- 0.1
time <- seq(0, 10, dt)
...
par(las=1, mar=c(2.5,5,1.5,1)+.1, lwd=2)
library("deSolve")
model <- function (time, y, parms) {
  with(as.list(c(y, parms)), {
    dN <- r * N * (1 - N / K)
...
... other solvers, fine-tuning and pitfalls, see [docs](cran.r-project.org/package=deSolve/)
...
$\rightarrow$ problems with precision, CPU effort, stability
...
- A basic Covid 19 model
- Basic idea of a lake: the Chemostat
- Lotka-Volterra's predator-prey interactions
- Reactive transport in a river
...
y <- c(
  Alg = 0.1, # algae,      mg/L carbon
  P   = 0.2  # phosphorus, mg/L P
) # in mg/L
...
\\
\frac{dPredator}{dt} &= g \cdot \mbox{Grazing} - \mbox{Mortality}
\end{align}
...
model <- function(t, n, parms) {
  with(as.list(c(n, parms)), {
    dn1 <- r1 * n1 - a13 * n1 * n3   # Prey1
    dn2 <- r2 * n2 - a24 * n2 * n4   # Prey2
    dn3 <- a13 * n1 * n3 - r3 * n3   # Predator1
    dn4 <- a24 * n2 * n4 - r4 * n4   # Predator2
