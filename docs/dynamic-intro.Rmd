...
```{r diag-im-ex, echo=FALSE, fig.width=14, fig.height=4}
...
par(mar=c(4, 5, 0, 1)+.1, las=1, cex.lab=2)
r    <- 0.5
N0   <- 10
dt   <- 0.1
time <- seq(0, 10, dt)
N <- N0 * exp(r * time)
plot(time, N, type="l", cex.axis=1.4)
```
```r
r    <- 0.5
N0   <- 10
...
```{r, ode-naive2, eval=TRUE, echo=FALSE, fig.width=6, fig.height=4}
par(las=1, cex.lab=1.5, cex.axis=1.5, mar=c(5,5,1.5,0)+.1)
library("deSolve")
model <- function (time, y, parms) {
  with(as.list(c(y, parms)), {
    dN <- r * N * (1 - N / K)
...
```{r diag-resource-limited, echo=FALSE, eval=TRUE, fig.height=3, fig.width=14}
library(DiagrammeR)
...
```{r diag-sir, echo=FALSE, fig.width=14, fig.height=4}
grViz("digraph feedback {
         graph [rankdir = 'LR']
...
```{r SIR-solution, echo=TRUE, fig.height=6, fig.width=14}
SIR <- function(t, state, parameters) {
  with (as.list(c(state, parameters)), {
    ...
    list(c(dS.dt, dI.dt, dR.dt), # the time derivatives
...
```{r Chemostat-dynamic, echo=TRUE, fig.height=6, fig.width=14, dev.args=list(pointsize=20)}
library("deSolve")
library("rootSolve")
...
plot(out, mfrow=c(1, 3), las=1, cex.axis=1.4)
```
...
```{r chemostat-equilibrium, echo=TRUE, fig.height=12, fig.width=14, }
state <- data.frame(
...
```{r diag-LV, echo=FALSE, fig.width=14, fig.height=4}
grViz("digraph lotka {
         graph [rankdir = 'LR']
           node [shape = box, penwidth=2, fontname = 'Helvetica']
             Predator, Prey
